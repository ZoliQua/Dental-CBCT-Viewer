...
function implant(entry: Vec3, axis: Vec3 = [0, 0, 1], sleeveDiameter = 5): GuideCheckImplant {
  return { entry, axis, length: 10, sleeveDiameter, sleeveOffset: 9, sleeveHeight: 5 };
}
...
describe('validateGuide', () => {
  ...
  it('warns when the working bore is too narrow for a bur', () => {
    const issues = validateGuide({ implants: [implant([0, 0, 0], [0, 0, 1], 1.5)], params: params() });
    expect(issues.some((i) => i.code === 'narrowChannel')).toBe(true);
  });

  it('sleeve wall no longer shrinks the drill channel', () => {
    // Regression: sleeveWallMm builds the seat OUTWARD; it must not narrow the bore.
    const issues = validateGuide({ implants: [implant([0, 0, 0])], params: params({ sleeveSeat: true, sleeveWallMm: 1.8 }) });
    expect(issues.some((i) => i.code === 'narrowChannel')).toBe(false);
  });
  ...
  it('flags a fragile web between seat pockets that the drill bores alone miss', () => {
    // 7 mm apart: drill bores (Ø5.1) leave a 1.9 mm web — fine. The seat
    // pockets (Ø6.9) leave only 0.1 mm — fragile. Only seat mode should warn.
    const pair = [implant([0, 0, 0]), implant([7, 0, 0])];
    const sleeveless = validateGuide({ implants: pair, params: params({ sleeveSeat: false }) });
    expect(sleeveless.some((i) => i.code === 'boresClose')).toBe(false);
    const seated = validateGuide({ implants: pair, params: params({ sleeveSeat: true }) });
    expect(seated.some((i) => i.code === 'boresClose')).toBe(true);
  });
